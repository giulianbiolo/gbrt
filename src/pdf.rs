...
impl HittablePDF {
    pub fn new(origin: Point3, ptr: Arc<dyn Hittable>) -> Self { HittablePDF { origin, ptr } }
}
...
impl MixturePDF {
    pub fn new(p0: Arc<dyn PDF>, p1: Arc<dyn PDF>) -> Self { MixturePDF { p0, p1 } }
}

impl PDF for MixturePDF {
    fn value(&self, direction: &Vec3A) -> f32 { 0.5 * self.p0.value(direction) + 0.5 * self.p1.value(direction) }
    fn generate(&self) -> Vec3A {
        if utility::random_f32() < 0.5 { self.p0.generate() } else { self.p1.generate() }
    }
}
